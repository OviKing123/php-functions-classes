...
class YahooSearch {
	...
	public function __construct( $search = null, $domain = null, $language = null, $page = null ) {
		...
		$this->default->domain = 'YAHOO.COM';
		$this->default->domain_h = 'search.yahoo.com';
		$this->default->language = 'ENUS';
		$this->default->language_hl = 'en-US';
		$this->default->page = '1';
		$this->default->page_b = '0';
	...
	/**
	 * Força pasta de cache no local $_SERVER['DOCUMENT_ROOT'] . '/cache/classes/YahooSearch/'
	...
	protected function _setPage( $page ) {
		...
		$this->input->page = $this->___setPage( $page, $page_b );
		$this->input->page_b = $page_b;
		return $this;
	}
	 ...
	 * @param  [type] $page    [description]
	 * @param  [type] &$page_b [description]
	 * @return [type]          [description]
	 */
	protected function ___setPage( $page, &$page_b ) {
		if ( !is_numeric( $page ) || $page <= 0 || ( $page = trim( $page ) ) === '' ) {
			$page = $this->default->page;
			$page_b = $this->default->page_b;
			return $page;
		}
		return $this->____setPage( $page, $page_b );
	}
	 ...
	 * @return [type]          [description]
	 */
	protected function ____setPage( $page, &$page_b ) {
		$page_b = strval( $page - 1 );
		return $page;
	}
	...
	protected function ____isDomain( &$domain, &$domain_h ) {
		...
		$domains = isset( $domains ) ? $domains : array(
			'WWW.YAHOO.COM' => array( 'YAHOO.COM', 'search.yahoo.com' ),
			'YAHOO.COM' => array( 'YAHOO.COM', 'search.yahoo.com' ),
			'ENUS' => array( 'YAHOO.COM.BR', 'search.yahoo.com' ),
			'EN' => array( 'YAHOO.COM.BR', 'search.yahoo.com' ),
			'BR.YAHOO.COM' => array( 'YAHOO.COM.BR', 'br.search.yahoo.com' ),
			'YAHOO.COM.BR' => array( 'YAHOO.COM.BR', 'br.search.yahoo.com' ),
			'PTBR' => array( 'YAHOO.COM.BR', 'br.search.yahoo.com' ),
			'BR' => array( 'YAHOO.COM.BR', 'br.search.yahoo.com' ),
		);
	...
	protected function _http_search() {
		...
		if ( defined( 'ABSPATH' ) && !$this->settings->cache_root ) {
			$this->http->search->cache_folder = ABSPATH . 'wp-content/cache/class-yahoo-search/search/';
		} else {
	...
	protected function _http_search_extract_url() {
		...
		if ( $this->input->page === '1' ) {
			$result = sprintf( 'https://%s/search?p=%s', $this->input->domain_h, $this->input->search_encode );
		} else {
			$result = sprintf( 'https://%s/search?p=%s&b=%s', $this->input->domain_h, $this->input->search_encode, $this->input->page_b );
		}
	...
	protected function _parse_search_get_results() {
		...
		$needles_open = array(
			...
			'<h2 class="off-left">Search Shortcut</h2>',
			'<div id="web" role="main">',
		);

		$needles_close = array(
			'</div></div><!-- end section main --><h2 class="off-left">More search results</h2>',
			'</div></div><!-- end section main --><h2',
			'<!-- end section main -->',
		);
		...
		$pattern = '/\<li\b[^\x3e]*\>.*?\<h3\b[^\x3e]*\>.*?\<a\b[^\x3e]*href\=\x22([^\x22]+)\x22[^\x3e]*\>(.+)\<\/a\>[\x00-\x20\x7f]*\<\/h3\>.*?\<div\b[^\x3e]*class\x3d\x22abstr\x22[^\x3e]*\>(.+)\<\/div\>\<\/div\>\<\/li\>/';
	...
	/**
	 * [_parse_search_fix_urls description]
	 * @return [type] [description]
	 */
	protected function _parse_search_fix_urls() {

		foreach ( $this->output->urls as $key => $value ) {
			if ( preg_match( '/RU\x3d([^\x2f]+)\x2fRK\x3d0/', $value, $match ) ) {
				$value = urldecode( $match[1] );
			}
			$this->output->urls[$key] = $value;
		}
