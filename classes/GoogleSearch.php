...
class GoogleSearch {
	...
	public function __construct( $search = null, $domain = null, $language = null, $page = null ) {
		...
		$this->default->domain = 'GOOGLE.COM';
		$this->default->domain_h = 'www.google.com';
	...
	/**
	 * Força pasta de cache no local $_SERVER['DOCUMENT_ROOT'] . '/cache/classes/GoogleSearch/'
	...
	protected function ____isDomain( &$domain, &$domain_h ) {
		...
		$domains = isset( $domains ) ? $domains : array(
			'WWW.GOOGLE.COM' => array( 'GOOGLE.COM', 'www.google.com' ),
			'GOOGLE.COM' => array( 'GOOGLE.COM', 'www.google.com' ),
			'WWW.GOOGLE.COM.BR' => array( 'GOOGLE.COM', 'www.google.com' ),
			'GOOGLE.COM.BR' => array( 'GOOGLE.COM.BR', 'www.google.com.br' ),
		);
	...
	protected function ____isLanguage( &$language, &$language_hl ) {
		...
		$languages = isset( $languages ) ? $languages : array(
			'EN-US' => array( 'ENUS', 'en' ),
			'ENUS' => array( 'ENUS', 'en' ),
			'PT-BR' => array( 'PTBR', 'pt-BR' ),
			'PTBR' => array( 'PTBR', 'pt-BR' ),
		);
	...
	protected function _run( $search, $domain, $language, $page ) {
		...
		!isset( $this->input->domain ) && $this->setDomain(null);
	...
	protected function _http_search() {
		...
		if ( defined( 'ABSPATH' ) && !$this->settings->cache_root ) {
			$this->http->search->cache_folder = ABSPATH . 'wp-content/cache/class-google-search/search/';
		} else {
		...
		$this->http->search->headers = array(
			...
			'Referer: http://' . $this->input->domain_h . '/',
		);
	...
	protected function _http_search_extract_url() {
		...
		if ( $this->input->page === '1' ) {
			$result = sprintf( 'https://%s/search?q=%s&hl=%s&gws_rd=cr', $this->input->domain_h, $this->input->search_encode, $this->input->language_hl );
		} else {
			$result = sprintf( 'https://%s/search?q=%s&hl=%s&gws_rd=cr', $this->input->domain_h, $this->input->search_encode, $this->input->language_hl );
		}
	...
	protected function _parse_search_get_results() {
		...
		$gcount = substr_count( $body, '<li class="g"' );
		$acount = substr_count( $body, '<li class="action-menu-item ab_dropdownitem"' );

		if ( $gcount === $acount ) {
			$body = str_replace( '<li class="g"', "\x0a" . '<li class="g"', $body );
		} else {
			$body = str_replace( '<li', "\x0a" . '<li', $body );
		}

		$pattern = '/\<h3\b[^\x3e]*\>.*?\<a\b[^\x3e]*href\=\x22([^\x22]{2,})\x22[^\x3e]*\>\x20*([^\x20].+[^\x20])\x20*\<\/a\>\<\/h3\>.*?\<cite\b[^\x3e]*\>\x20*(?:[^\x20].+[^\x20])\x20*\<\/cite\>.*?\<a\b[^\x3e]*href\=\x22(?:[^\x22]{2,})\x22[^\x3e]*\>.*?\<span\b[^\x3e]*\>(.+)\<\/span\>.*?\<\/li\>/';
		...
		if ( count( $matches[0] ) !== $gcount ) {
			$this->___setErrorMethod( __FUNCTION__, 'count( $matches[0] ) !== $gcount' );
			return false;
		}
