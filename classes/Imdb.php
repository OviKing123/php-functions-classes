...
class Imdb {
	...
	protected function _parse() {
		...
		foreach ( $this->imdb_languages as $this->imdb_language ) {
			...
			$this->raw->original_title[$this->imdb_language] = null;
			$this->raw->english_title[$this->imdb_language] = null;
			$this->raw->brazil_title[$this->imdb_language] = null;
			$this->raw->portugal_title[$this->imdb_language] = null;

			/*
			$this->raw->original_title_2[$this->imdb_language] = null;
			$this->raw->english_title_2[$this->imdb_language] = null;
			$this->raw->brazil_title_2[$this->imdb_language] = null;
			$this->raw->portugal_title_2[$this->imdb_language] = null;

			$this->raw->original_title_3[$this->imdb_language] = null;
			$this->raw->english_title_3[$this->imdb_language] = null;
			$this->raw->brazil_title_3[$this->imdb_language] = null;
			$this->raw->portugal_title_3[$this->imdb_language] = null;

			$this->raw->original_title_4[$this->imdb_language] = null;
			$this->raw->english_title_4[$this->imdb_language] = null;
			$this->raw->brazil_title_4[$this->imdb_language] = null;
			$this->raw->portugal_title_4[$this->imdb_language] = null;
			//*/
			...
			if ( $imdb_page_releaseinfo_table_id_akas_open_pos[$this->imdb_language] !== false ) {
				...
				foreach ( $imdb_page_releaseinfo_table_akas_titles_matches[$this->imdb_language][1] as $key[$this->imdb_language] => $country[$this->imdb_language] ) {
					...
					if ( $country[$this->imdb_language] === 'Brazil' ) {
						if ( isset( $this->raw->brazil_title[$this->imdb_language] ) ) {
							...
							if ( $this->id === 'tt0087727' ) {
								$this->raw->brazil_title[$this->imdb_language] = 'Braddock: O Super Comando';
								continue;
							}
						...
						$this->raw->brazil_title[$this->imdb_language] = $imdb_page_releaseinfo_table_akas_titles_matches[$this->imdb_language][2][$key[$this->imdb_language]];
						$imdb_page_releaseinfo_has_brazil_title[$this->imdb_language] = true;
						continue;
					}
					if ( $country[$this->imdb_language] === 'Portugal' ) {
						if ( isset( $this->raw->portugal_title[$this->imdb_language] ) ) {
						...
						$this->raw->portugal_title[$this->imdb_language] = $imdb_page_releaseinfo_table_akas_titles_matches[$this->imdb_language][2][$key[$this->imdb_language]];
						...
						if ( in_array( $country[$this->imdb_language], array( 'Brazil (working title)', 'Brazil (DVD title)', 'Brazil (alternative title)', 'Brazil (original subtitled version)' ) ) ) {
							if ( isset( $this->raw->brazil_title[$this->imdb_language] ) ) {
								...
								if ( $this->id === 'tt1647668' ) {
									$this->raw->brazil_title[$this->imdb_language] = 'Um Braço de Um Milhão de Dólares';
									continue;
								}
								if ( isset( $imdb_skip_titles ) && $imdb_skip_titles ) {
									$imdb_is_skipped = true;
	...
	protected function _setImage() {
		...
		if ( $imdb_page_index_td_id_img_primary_open_pos[$this->imdb_language] !== false ) {
			...
			if ( preg_match( '/\<img\b[^\x3e]*src\=(?:\x22([^\x22]+)\x22|\x27([^\x27]+)\x27|([^\x20\x22\x27]+)) itemprop=\"image\"/', $imdb_page_index_td_id_img_primary_html[$this->imdb_language], $imdb_page_index_image_matches[$this->imdb_language] ) ) {
				$this->raw->image[$this->imdb_language] = $imdb_page_index_image_matches[$this->imdb_language][1];
				$this->raw->image_full[$this->imdb_language] = $this->raw->image[$this->imdb_language];
				$this->raw->image_320[$this->imdb_language] = $this->raw->image[$this->imdb_language];
				$this->raw->image_full[$this->imdb_language] = preg_replace( '/([@]+)(\._(.+)\.jpg)/', '$1.jpg', $this->raw->image_full[$this->imdb_language] );
				$this->raw->image_320[$this->imdb_language] = preg_replace( '/V(1)_S(Y)317_CR(0)\,(0)\,214\,317_AL_\.jpg/', 'V$1_S$2;;;474_CR$3,$4,320,474_AL_.jpg', $this->raw->image_320[$this->imdb_language] );
				$this->raw->image_320[$this->imdb_language] = str_replace( ';;;', '', $this->raw->image_320[$this->imdb_language] );
			}
		}
	...
	protected function _setDocumentTitle() {

		if ( !isset( $this->imdb_index_body[$this->imdb_language] ) || !isset( $this->imdb_language ) ) {
			$this->_setError( 'Protected_SetDocumentTitle_Index_Body_Or_Language_Undefined' );
			return false;
		}
		...
		if ( !preg_match_all( '/\<h1 class\=\"header\"\>[\x20]*\<span class\=\"itemprop\" itemprop\=\"name\"\>[\x20]*([^\x20]+[^\x3c]+[^\x20]+)[\x20]*\<\/span\>/', $this->imdb_index_body[$this->imdb_language], $imdb_page_index_h1_title_matches[$this->imdb_language] ) ) {
			$this->_setError( 'Error' . __LINE__ );
			#echo '<h2>Imdb - Index - H1 Title - 1 - ' . $url . '</h2>';
			#echo '<textarea>' . $this->imdb_index_body[$this->imdb_language] . '</textarea>';
			return false;
		}

		if ( !isset( $imdb_page_index_h1_title_matches[$this->imdb_language][1] ) ) {
			$this->_setError( 'Error' . __LINE__ );
			#echo '<h2>Imdb - Index - H1 Title - 2 - ' . $url . '</h2>';
			return false;
		}

		if ( count( $imdb_page_index_h1_title_matches[$this->imdb_language][1] ) !== 1 ) {
			$this->_setError( 'Error' . __LINE__ );
			#echo '<h2>Imdb - Index - H1 Title - 3 - ' . $url . '</h2>';
			return false;
		}
		...
		if ( !preg_match_all( '/\<title\>[\x20]*([^\x20]+[^\x3c]+[^\x20]+)[\x20]*\<\/title\>/', $this->imdb_index_body[$this->imdb_language], $imdb_page_index_document_title_matches[$this->imdb_language] ) ) {
			$this->_setError( 'Protected_SetDocumentTitle_Preg_Match_All_False_' . $this->imdb_language );
			return false;
		}

		if ( !isset( $imdb_page_index_document_title_matches[$this->imdb_language][1] ) ) {
			$this->_setError( 'Protected_SetDocumentTitle_Matches_Undefined_' . $this->imdb_language );
			return false;
		}

		if ( count( $imdb_page_index_document_title_matches[$this->imdb_language][1] ) !== 1 ) {
			$this->_setError( 'Protected_SetDocumentTitle_Count_Diff_1_' . $this->imdb_language );
			return false;
		}
		...
		if ( $prcount[$this->imdb_language] === 0 ) {
			$this->_setError( 'Protected_SetDocumentTitle_Preg_Replace_Count_Equal_Zero_' . $this->imdb_language );
			return false;
		}

		if ( $imdb_page_index_h1_title_matches[$this->imdb_language][1][0] !== $imdb_page_index_document_title_matches[$this->imdb_language][1][0] ) {
			if ( !isset( $imdb_bypass ) || !$imdb_bypass ) {
				$this->_setError( 'Protected_SetDocumentTitle_Titles_Dont_Match_' . $this->imdb_language );
				return false;
			}
		}
	...
	protected function _setGenres() {

		if ( !isset( $this->imdb_index_body[$this->imdb_language] ) || !isset( $this->imdb_language ) ) {
			$this->_setError( 'Protected_SetGenres_Index_Body_Or_Language_Undefined' );
			return false;
		}
		...
		if ( !preg_match_all( '/\<span class\=\"itemprop\" itemprop\=\"genre\"\>[\x20]*([^\x20]+[^\x3c]+[^\x20]+)[\x20]*\<\/span\>/', $this->imdb_index_body[$this->imdb_language], $imdb_page_index_genres_matches[$this->imdb_language] ) ) {
			$this->_setError( 'Protected_SetGenres_Preg_Match_All_False_' . $this->imdb_language );
			return false;
		}

		if ( !isset( $imdb_page_index_genres_matches[$this->imdb_language][1] ) ) {
			$this->_setError( 'Protected_SetGenres_Matches_Undefined_' . $this->imdb_language );
			return false;
		}

		$this->raw->genres[$this->imdb_language] = $imdb_page_index_genres_matches[$this->imdb_language][1];
		$this->raw->genres_str[$this->imdb_language] = implode( ', ', $this->raw->genres[$this->imdb_language] );

		#$imdb_generos[$this->imdb_language] = $this->_convertGenresToGeneros( $this->raw->genres[$this->imdb_language], true );
		#$imdb_generos_str[$this->imdb_language] = implode( ', ', $imdb_generos[$this->imdb_language] );

		return true;

	}

	protected function _setXYZ() {

		if ( !isset( $this->imdb_index_body[$this->imdb_language] ) || !isset( $this->imdb_language ) ) {
			$this->_setError( 'Protected_SetXyz_Index_Body_Or_Language_Undefined' );
			return false;
		}
		...
		if ( !$imdb_page_index_span_class_nobr_count[$this->imdb_language] ) {
			if ( !preg_match( '/\<span class\=\"nobr\"\>\([0-9]+\)\<\/span\>[\x09\x0a\x0b\x0c\x0d\x20]+\<\/h1\>/', $this->imdb_index_body[$this->imdb_language] ) ) {
				if ( !preg_match( '/\<span class\=\"nobr\"\>\(201[0-5]|200[0-9]|19[6-9][0-9]\)\<\/span\>[\x09\x0a\x0b\x0c\x0d\x20]*\<br\/\>\<span class\=\"title\-extra\" itemprop\=\"name\"\>/', $this->imdb_index_body[$this->imdb_language] ) ) {
					$this->_setError( 'Protected_SetXyz_Substr_Count_Preg_Match_Both_Error_' . $this->imdb_language );
					return false;
				}
			}
		...
		if ( $imdb_page_index_span_class_nobr_count[$this->imdb_language] !== 1 ) {
			$this->_setError( 'Protected_SetXyz_NoBr_Count_Diff_1_' . $this->imdb_language );
			return false;
		}
			...
			if ( !preg_match_all( '/\<span class\=\"nobr\"\>\((201[0-5]|200[0-9]|19[6-9][0-9])\)\<\/span\>[\x09\x0a\x0b\x0c\x0d\x20]+\<\/h1\>/', $this->imdb_index_body[$this->imdb_language], $imdb_page_index_years_nobr_matches[$this->imdb_language] ) ) {
				if ( !preg_match_all( '/<span class=\"nobr\"\>\((201[0-5]|200[0-9]|19[6-9][0-9])\)\<\/span\>[\x09\x0a\x0b\x0c\x0d\x20]*\<br\/>\<span class\="title\-extra\" itemprop\=\"name\"\>[\x09\x0a\x0b\x0c\x0d\x20]*\"?([^\x22\x3c]+)\"?[\x09\x0a\x0b\x0c\x0d\x20]*\<i\>[\x09\x0a\x0b\x0c\x0d\x20]*\(original title\)[\x09\x0a\x0b\x0c\x0d\x20]*\<\/i\>/', $this->imdb_index_body[$this->imdb_language], $imdb_page_index_years_nobr_matches[$this->imdb_language] ) ) {
					if ( !preg_match_all( '/tv_header\"\>[\s\S]+\<span class\=\"nobr\"\>\([0-9]+ (?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\. (201[0-5]|200[0-9]|19[6-9][0-9])\)\<\/span\>/', $this->imdb_index_body[$this->imdb_language], $imdb_page_index_years_nobr_matches[$this->imdb_language] ) ) {
						$this->_setError( 'Protected_SetXyz_Preg_Match_All_False_' . $this->imdb_language );
						return false;
					}
				}
			}
		...
		if ( preg_match_all( '/((3[0-1]|[1-2][0-9]|[1-9]) (January|February|March|April|May|June|July|August|September|October|November|December) (201[0-5]|200[0-9]|19[6-9][0-9])).+\=[\x22\x27]{0,1}((201[0-5]|200[0-9]|19[6-9][0-9])\-(1[0-2]|0[1-9])\-(3[0-1]|[1-2][0-9]|0[1-9]))[\x22\x27]{0,1}[^\x3e]*\/?\>[\x20]*\((West Germany|South Africa|USA|Brazil|Mexico|Croatia|Indonesia|Hong Kong|Spain|Norway|Portugal|France|Italy|Germany|Japan|Canada|Netherlands|Ireland|China|UK|Argentina|Belgium)\)[\x20]*(?:&ndash;)?[\x20]*\x3c/', $imdb_page_index_meta_datepublished_html[$this->imdb_language], $imdb_page_index_meta_datepublished_matches[$this->imdb_language], PREG_SET_ORDER ) ) {
			...
			$this->_setError( 'Protected_SetXyz_DatePublished_Not_Found_' . $this->imdb_language );
			return false;

		}

		$this->raw->release_year[$this->imdb_language] = $imdb_page_index_meta_datepublished_matches[$this->imdb_language][0][4];
		...
		$this->raw->release_date[$this->imdb_language] = $imdb_page_index_meta_datapublicada[$this->imdb_language];

		if ( isset( $this->raw->release_date_country[$country_key[$this->imdb_language]] ) ) {
			if ( $this->raw->release_date_country[$country_key[$this->imdb_language]] !== $imdb_page_index_meta_datapublicada[$this->imdb_language] ) {
				$this->_setError( 'Protected_SetXyz_Release_Date_Country_Dont_Match_' . $this->imdb_language );
				return false;
			}
		}

		$this->raw->release_date_country[$country_key[$this->imdb_language]] = $imdb_page_index_meta_datapublicada[$this->imdb_language];
	...
	protected function _setDuration() {
		...
		if ( !isset( $this->imdb_index_body[$this->imdb_language] ) || !isset( $this->imdb_language ) ) {
			$this->_setError( 'Protected_SetDuration_Index_Body_Or_Language_Undefined_' );
			return false;
		}
		...
		if ( $imdb_page_index_infobar_open_pos[$this->imdb_language] === false || $imdb_page_index_infobar_close_pos[$this->imdb_language] === false ) {
			$this->_setError( 'Protected_SetDuration_Open_Or_Close_Equal_False_' . __LINE__ );
			return false;
		}
		...
		if ( ( !isset( $this->settings->duration_is_manual ) || !$this->settings->duration_is_manual ) && !isset( $imdb_page_index_duration_matches[$this->imdb_language][0][1] ) ) {
			$this->_setError( 'Protected_SetDuration_Matches_Undefined_' . __LINE__ );
			return false;
		}

		$imdb_duration[$this->imdb_language] = isset( $imdb_page_index_duration_matches[$this->imdb_language][0][1] ) ? $imdb_page_index_duration_matches[$this->imdb_language][0][1] : null;

		return true;

	}

	protected function _setRating() {

		if ( !isset( $this->imdb_index_body[$this->imdb_language] ) || !isset( $this->imdb_language ) ) {
			$this->_setError( 'Protected_SetRating_Index_Body_Or_Language_Undefined_' );
			return false;
		}
		...
		if ( !( preg_match_all( '/<div class\=\"titlePageSprite star\-box\-giga\-star\"\>[\x20]*([0-9\.]+)[\x20]*/', $this->imdb_index_body[$this->imdb_language], $imdb_page_index_rating_matches[$this->imdb_language], PREG_SET_ORDER ) || preg_match_all( '/\<span itemprop\=\"ratingValue\"\>[\x20]*([0-9\.]+)[\x20]*\<\/span\>/', $this->imdb_index_body[$this->imdb_language], $imdb_page_index_rating_matches[$this->imdb_language], PREG_SET_ORDER ) ) ) {
			...
			$this->raw->rating[$this->imdb_language] = null;
		...
		if ( count( $imdb_page_index_rating_matches[$this->imdb_language] ) !== 1 ) {
			$this->_setError( 'Protected_SetRating_Count_Diff_1_' . __LINE__ );
			return false;
		}

		if ( !isset( $imdb_page_index_rating_matches[$this->imdb_language][0][1] ) ) {
			$this->_setError( 'Protected_SetRating_Matches_Undefined_' . __LINE__ );
			return false;
		}

		$this->raw->rating[$this->imdb_language] = $imdb_page_index_rating_matches[$this->imdb_language][0][1];
